...
# --- SIDEBAR ---
with st.sidebar:
    st.button("🗑️ PULISCI DATI", on_click=clear_form, help="Cancella tutti i dati inseriti e resetta il modulo")
    st.divider()
    ...
    rgr = st.text_input("Numero R.G.R.", value="1234/2023", key="rgr")
    sez = st.text_input("Sezione", key="sez")
    u_check = st.checkbox("Includere data udienza?", key="u_check")
    u_data = ""
    if u_check:
        u_data = st.date_input("Data Udienza", datetime.date.today(), key="u_data").strftime("%d.%m.%Y")

    st.header("Valore e Spese")
    indet = st.checkbox("Valore Indeterminato", key="indet")
    if indet:
        ...
        v_lite = st.number_input("Valore della lite (€)", min_value=0.0, value=15000.0, key="v_lite")
        v_calc = float(v_lite)
        val_txt = ""

    cut_val = st.number_input("Contributo Unificato (C.U.T.) €", min_value=0.0, value=0.0, key="cut_val")

    st.header("Cliente")
    gender = st.radio("Genere", ["Femminile", "Maschile"], key="gender")
    nome_cli = st.text_input("Nome Cliente", value="Federica Benzi", key="nome_cli")
    luogo_n = st.text_input("Luogo di Nascita", value="Montevideo (EE)", key="luogo_n")
    data_n = st.text_input("Data di Nascita", value="21/03/1959", key="data_n")
    cf_cli = st.text_input("C.F. Cliente", value="BNZFRC59C61Z613C", key="cf_cli")
    res_cli = st.text_input("Residenza", value="Sagliano Micca (BI), via Grosso n. 8", key="res_cli")
...
def create_doc():
    ...
    p_seg = doc.add_paragraph("la seguente nota spese:")
    ...
    add_row = add_r("TOTALE IMPONIBILE", f"€ {imp:,.2f}", True)
...
if st.button("🚀 GENERA DOCUMENTO"):
    try:
        st.download_button("📥 Scarica il documento Word", create_doc(), f"Nota_{nome_cli.replace(' ','_')}.docx")
